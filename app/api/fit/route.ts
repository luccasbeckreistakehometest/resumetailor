...
import { fitHash, fitLimitFor, fitUsage, getCachedFit, saveFit, serialiseFit } from "@/lib/server/fit";
...
export async function POST(request: Request) {
  ...
  return withOwner(async (owner) => {
    ...
    const limit = fitLimitFor(owner.userId);
    const hash = fitHash(posting, resume, lang);
    const cached = getCachedFit(hash);
    if (cached) return { body: serialiseFit(cached, true, fitUsage(owner.key, Date.now(), limit)) };
    ...
    try {
      const usage = fitUsage(owner.key, Date.now(), limit);
      ...
      return { body: serialiseFit(row, false, fitUsage(owner.key, Date.now(), limit)) };
    } finally {
      slot.release();
    }
  });
...
export async function GET() {
  return withOwner(async (owner) => ({ body: fitUsage(owner.key, Date.now(), fitLimitFor(owner.userId)) }));
}
