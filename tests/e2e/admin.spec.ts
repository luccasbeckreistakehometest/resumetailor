...
test("admin sees totals, users, and the first-session timeline; users do not", async ({ page }) => {
  ...
  // The admin's sections are a real tablist now (docs/DESIGN.md §11.5), not loose buttons.
  await page.getByRole("tab", { name: /^users|usuários|usuarios/i }).click();
  await expect(page.getByTestId("admin-table")).toContainText(user.email);
  await page.getByRole("tab", { name: /first sessions|primeiras/i }).click();
