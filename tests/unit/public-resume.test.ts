...
describe("access rules", () => {
  it("only an enabled page opens; a PIN page opens only once verified; missing and off look alike", () => {
...
describe("slug and contact stripping", () => {
  it("slugifies accented names and falls back when nothing is left", () => {
  ...
  it("removes emails, phones and profile links but keeps everything else", () => {
  ...
  it("drops a line that was only contact details", () => {
...
describe("publishing", () => {
  ...
  it("creates the page off by default with a stable slug, then applies patches", () => {
    const u = user(); const g = gen(u.id); unlockGeneration(g.id, u.id);
    const first = upsertPublic(g.id, u.id, g.title, {});
    expect(first.enabled).toBe(0);
    expect(first.slug).toMatch(/^alex-ribeiro-[0-9a-f]{6}$/);
    const on = upsertPublic(g.id, u.id, g.title, { enabled: true, template: "elegant", hideContact: true, indexable: true });
  ...
  it("hashes the PIN, verifies it, and a new PIN invalidates the old browser token", () => {
    const u = user(); const g = gen(u.id); unlockGeneration(g.id, u.id);
    const row = upsertPublic(g.id, u.id, g.title, { enabled: true, pin: "2468" });
    expect(row.pinHash).not.toContain("2468");
    expect(verifyPin(row, "2468")).toBe(true);
    expect(verifyPin(row, "0000")).toBe(false);
    const token = pinToken(row);
    expect(pinTokenValid(row, token)).toBe(true);
    expect(pinTokenValid(row, token.slice(0, -1) + "x")).toBe(false);
    const changed = upsertPublic(g.id, u.id, g.title, { pin: "abcd12" });
    expect(pinTokenValid(changed, token)).toBe(false);
    expect(pinTokenValid(changed, pinToken(changed))).toBe(true);
    const cleared = upsertPublic(g.id, u.id, g.title, { pin: null });
    expect(cleared.pinHash).toBeNull();
    expect(pinTokenValid(cleared, pinToken(changed))).toBe(false);
    expect(() => upsertPublic(g.id, u.id, g.title, { pin: "12" })).toThrow("pin");
    expect(() => upsertPublic(g.id, u.id, g.title, { pin: "has space" })).toThrow("pin");
  });

  it("counts views, and the page disappears with the kit", () => {
    const u = user(); const g = gen(u.id); unlockGeneration(g.id, u.id);
    const row = upsertPublic(g.id, u.id, g.title, { enabled: true });
    ...
    const row2 = upsertPublic(g2.id, u.id, g2.title, { enabled: true });
    deletePublic(g2.id);
    expect(getPublicBySlug(row2.slug)).toBeNull();
    expect(serialise(getGeneration(g2.id)!).publicResume).toBeNull();
  });

  it("a locked kit exposes nothing about publishing", () => {
    const u = user(); const g = gen(u.id);
    upsertPublic(g.id, u.id, g.title, { enabled: true });   // the route refuses this; the serialiser hides it regardless
