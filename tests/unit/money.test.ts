...
describe("credits", () => {
  it("grants the signup credit and records it in the ledger", async () => {
    const u = await user();
...
describe("payments", () => {
  it("grants exactly once for the same provider payment id", async () => {
    const u = await user();
  ...
  it("holds pending payments without credits and settles them later", async () => {
    const u = await user();
...
describe("kits", () => {
  ...
  it("unlocks once, spends one credit, and refuses a second spend", async () => {
    const u = await user();
  ...
  it("fails cleanly with no credits and does not open the kit", async () => {
    const u = await user();
  ...
  it("cannot unlock someone else's kit", async () => {
  ...
  it("anonymous work is claimed on signup", async () => {
    const anon = "anon_abc";
...
describe("auth", () => {
  it("round-trips a session and rejects a tampered one", async () => {
    const token = signSession({ userId: "u1", role: "user", sv: 0 });
    expect(verifySession(token)?.userId).toBe("u1");
    expect(verifySession(token.slice(0, -2) + "xx")).toBeNull();
  });
  it("hashes passwords with a per-user salt", async () => {
    const a = await hashPassword("password123"), b = await hashPassword("password123");
    expect(a).not.toEqual(b);
    expect(await verifyPassword("password123", a)).toBe(true);
    expect(await verifyPassword("password124", a)).toBe(false);
  });
});
