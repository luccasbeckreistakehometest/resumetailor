...
export function Field({
  ...
  return (
    <div className={`flex flex-col gap-[var(--s-3)] ${className}`}>
      <label htmlFor={id} className="flex items-baseline gap-[var(--s-3)] font-sans text-[length:var(--density-label)] font-medium text-[color:var(--ink-2)]">
        {label}
        {required && <span className="text-[color:var(--mark)]" aria-hidden>·</span>}
        {optional && <span className="text-[color:var(--ink-muted)] font-normal">optional</span>}
      </label>
      ...
      {(error || help || count) && (
        <div className="flex items-baseline justify-between gap-[var(--s-4)]">
          <div className="min-w-0">
            {error && (
              <p id={errorId} className="flex items-center gap-[var(--s-2)] font-sans text-[length:var(--ui-13)] text-[color:var(--mark)]">
                <Icon name="flag" size={16} /> {error}
              </p>
            )}
            {help && !error && <p id={helpId} className="font-sans text-[length:var(--ui-13)] leading-[var(--ui-13-lh)] text-[color:var(--ink-muted)]">{help}</p>}
          </div>
          {count && (
            <span
...
const CONTROL =
  "w-full rounded-[var(--r-1)] border bg-[var(--field)] px-[var(--s-4)] font-sans " +
  "text-[length:var(--density-body)] text-[color:var(--ink)] " +
  ...
  "disabled:cursor-not-allowed disabled:bg-[var(--sunken)] disabled:border-[var(--rule)] disabled:text-[color:var(--ink-40)] " +
...
export function Select({ invalid, className = "", children, ...rest }: { invalid?: boolean } & SelectHTMLAttributes<HTMLSelectElement>) {
  return (
    <div className="relative">
      ...
      <Icon name="chevron-down" size={16} className="pointer-events-none absolute right-[var(--s-4)] top-1/2 -translate-y-1/2 text-[color:var(--ink-muted)]" />
    </div>
  );
}
...
function Tick({ type, label, className = "", ...rest }: { type: "checkbox" | "radio"; label: ReactNode } & Omit<InputHTMLAttributes<HTMLInputElement>, "type">) {
  ...
  return (
    <label className={`group inline-flex cursor-pointer items-start gap-[var(--s-3)] font-sans text-[length:var(--density-body)] text-[color:var(--ink)] has-[:disabled]:cursor-not-allowed has-[:disabled]:text-[color:var(--ink-40)] ${className}`}>
      <span className="relative mt-[2px] grid h-5 w-5 shrink-0 place-items-center">
        ...
        <span
          aria-hidden
          className={`pointer-events-none grid h-5 w-5 place-items-center border border-[var(--rule-field)] bg-[var(--field)] ${round}
            transition-colors duration-[var(--dur-1)]
            group-hover:border-[var(--ink-40)]
            peer-checked:border-[var(--ink)] peer-checked:bg-[var(--ink)] peer-checked:text-[color:var(--on-ink)]
            peer-checked:[&_*]:opacity-100
            peer-disabled:border-[var(--rule)] peer-disabled:bg-[var(--sunken)] peer-disabled:text-[color:var(--ink-40)]`}
        >
          {type === "checkbox"
