...
export function updateApplication(id: string, patch: ApplicationInput): ApplicationRow {
  ...
  db.transaction(() => {
    ...
    db.prepare(`UPDATE applications SET company=?, role=?, link=?, stage=?, notes=?, nextStepAt=?, generationId=?, appliedAt=?, interviewAt=?, offerAt=?, rejectedAt=?, stageChangedAt=?, updatedAt=? WHERE id=?`)
      .run(next.company, next.role, next.link, next.stage, next.notes, next.nextStepAt, next.generationId, next.appliedAt, next.interviewAt, next.offerAt, next.rejectedAt, next.stageChangedAt, at, id);
  })();
  return getApplication(id)!;
}
